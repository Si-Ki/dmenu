...
read_Xresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.normbg", "*", &type, &xval) == True)  /* normal background color */
			colors[SchemeNorm][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.normfg", "*", &type, &xval) == True)  /* normal foreground color */
			colors[SchemeNorm][ColFg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.selbg", "*", &type, &xval) == True)  /* selected background color */
			colors[SchemeSel][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.selfg", "*", &type, &xval) == True)  /* selected foreground color */
